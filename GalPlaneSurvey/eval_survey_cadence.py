...
def run_metrics(params):
    ...
    # Log file:
    logfile = open(os.path.join(params['output_dir'],runName+'_survey_cadence_data.txt'),'w')
    ...
    logfile.write('# Col 9: VIM(1/4cadence)\n')
    logfile.write('# Col 10: VIM(1/2cadence)\n')
    logfile.write('# Col 11: VIM(3/4cadence)\n')
    logfile.write('# Col 12: %OfIdeal(SeasonVisibilityGapsMetric)\n')
    logfile.write('# Col 13: %OfIdeal(Priority*VIM)\n')
    logfile.write('# Col 14: VIP(1/4cadence)\n')
    logfile.write('# Col 15: VIP(1/2cadence)\n')
    logfile.write('# Col 16: VIP(3/4cadence)\n')
    logfile.write('# Col 17: medianVIM\n')
    logfile.write('# Col 18: stddevVIM\n')

    debug = open(os.path.join(params['output_dir'],'survey_cadence_debug.log'),'w')
    ...
    for column in map_data_table.columns:
        ...
        for i in range(0,len(tau_obs),1):
            ...
            outputName1 = 'GalPlaneVisitIntervalsTimescales_'+mapName+'_Tau_'+str(tau_obs[i]).replace('.','_')
            ...
            if plot_png:
                plot_cumulative_VIM(params,runName,mapName,map_data,bundleDict,desired_healpix)

            logfile.write(runName+' '+mapName+' '+str(FoM.tau)+' '+str(FoM.tau_var)+' '+\
                            str(FoM.sumVIM)+' '+str(FoM.sumSVGM)+' '+str(FoM.sumVIP)+' '+\
                            str(FoM.percent_sumVIM)+' '+str(FoM.VIMdecayFn14)+' '+\
                            str(FoM.VIMdecayFn12)+' '+str(FoM.VIMdecayFn34)+' '+\
                            str(FoM.percent_sumSVGM)+' '+\
                            str(FoM.percent_sumVIP)+' '+str(FoM.VIPdecayFn14)+' '+\
                            str(FoM.VIPdecayFn12)+' '+str(FoM.VIPdecayFn34)+' '+\
                            str(FoM.medianVIM)+' '+str(FoM.stddevVIM)+'\n')
...
class TimeFiguresOfMerit:
    def __init__(self):
        ...
        self.percent_sumSVGM = None
        self.percent_sumVIP = None
...
def eval_metrics_by_region(params,bundleDict,map_data,runName,mapName,tau,
    ...
    FoM = TimeFiguresOfMerit()
    FoM.tau = tau
    FoM.tau_var = tau_var

    FoM = calc_VIM(params,runName,mapName,tau,bundleDict,desired_healpix,FoM)
    FoM = calc_SVGM(params,runName,mapName,tau,bundleDict,desired_healpix,FoM,datalog=datalog)
    FoM = calc_VIP(params,runName,mapName,map_data,tau,bundleDict,desired_healpix,FoM)
...
def calc_VIM(params,runName,mapName,tau,bundleDict,desired_healpix,FoM):
    ...
    if plot_png:
        file_name = os.path.join(params['output_dir'], runName+'_'+mapName+'_'+str(tau)+'_calcVIM.png')
